...
RUTA_SALIDA = './modelo_entrenado_mnist_cnn13/'
RUTA_SALIDA_C = 'modelo_entrenado_mnist_cnn13/'
...
with tf.Session() as sess:
    ...
    nn1 = tf.get_default_graph().get_tensor_by_name('nn1:0')
    conv1 = tf.get_default_graph().get_tensor_by_name('conv1:0')

    maxPool = tf.get_default_graph().get_tensor_by_name('maxPool_1:0')
    ...
    with open("./ficheros_c/"+RUTA_SALIDA_C+"wn1.h", 'w') as file:
        NUM_CONEXIONES_POR_NEURONA = 14*14*4
        string = "float wn1[] = {";
        for k in range(10):
    ...
    print(sess.run(nn1, feed_dict={X: np.expand_dims(mnist.test.images[0,:], axis=0)}))

    a = sess.run(maxPool, feed_dict={X: np.expand_dims(mnist.test.images[0,:], axis=0)})
    print(np.max(a))

    a = sess.run(conv1, feed_dict={X: np.expand_dims(mnist.test.images[0,:], axis=0)})
    print(np.max(a))
