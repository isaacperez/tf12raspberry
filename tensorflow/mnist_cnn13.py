...
wn1 = tf.Variable(tf.truncated_normal([14*14*4, 10], stddev=0.1), name="W_n1")
...
# Conv2
maxPool = tf.nn.max_pool(conv1r, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
maxPool = tf.identity(maxPool, name="maxPool")
maxPool_1D = tf.reshape(maxPool, [-1, 14*14*4])

# NN
nn1 = tf.matmul(maxPool_1D, wn1)
