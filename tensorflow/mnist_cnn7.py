...
RUTA_SALIDA = './modelo_entrenado_mnist_cnn7/'
...
wn1 = tf.Variable(tf.truncated_normal([3, 10], stddev=0.1), name="W_n1")
...
# AVG_GLOBAL
pool = tf.nn.avg_pool(conv1r, ksize=[1, 28, 28, 1], strides=[1, 1, 1, 1], padding='VALID')
pool = tf.identity(pool, name="pool")

pool1D = tf.reshape(pool, shape=[-1, 3])
