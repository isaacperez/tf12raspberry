...
int main()
{
  ...
  float *img_max_pool_2x2_S2_ptr = (float *) malloc(9 * 9 * 3 * sizeof(float)); // 7x7x3 con padding de 1 => 9x9x3
  ...
  // Max Pool
  t = clock();
  img_max_pool_2x2_S2_ptr = Max_Pooling_2x2_S2(img_conv_7x7_S2_ptr, NUM_FILAS, NUM_COLUMNAS, NUM_CANALES, STEP_FILA, STEP_COLUMNA, PADDING, 1, img_max_pool_2x2_S2_ptr);
  t = clock() - t;
  time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
  ...
  printf("\n======== Max_Pooling_2x2_S2(ReLU(suma_vector_con_tensor_por_canales(conv_7x7_S2(IMAGEN INICIAL CON PADDING 2 POR CADA LADO, Wn1), bc1))) ========\n");
  printf("Max_Pooling_2x2_S2() tardó %f segundos en ejecutarse \n", time_taken);
  //visualizarImagen(img_max_pool_2x2_S2_ptr, NUM_FILAS, NUM_COLUMNAS, NUM_CANALES, STEP_FILA, STEP_COLUMNA);


  // PRODUCTO TENSORES
  t = clock();
  producto_tensores_ptr = producto_tensores(img_max_pool_2x2_S2_ptr, wn1_ptr, producto_tensores_ptr, NUM_FILAS, NUM_COLUMNAS, NUM_CANALES, STEP_FILA, STEP_COLUMNA, 10);
  t = clock() - t;
  time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds

  printf("\n======== producto_tensores(Max_Pooling_2x2_S2(ReLU(suma_vector_con_tensor_por_canales(conv_7x7_S2(IMAGEN INICIAL CON PADDING 2 POR CADA LADO, Wn1), bc1)))) ========\n");
  ...
  printf("\n======== suma_vector_con_tensor_por_canales(producto_tensores(Max_Pooling_2x2_S2(ReLU(suma_vector_con_tensor_por_canales(conv_7x7_S2(IMAGEN INICIAL CON PADDING 2 POR CADA LADO, Wn1), bc1))))) ========\n");
  printf("suma_vector_con_tensor_por_canales() tardó %f segundos en ejecutarse \n", time_taken);
  visualizarImagen(producto_tensores_ptr, 1, 1, 10, 1, 1);
