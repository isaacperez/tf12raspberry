...
int main()
{
  ...
  float *img_segunda_conv_ptr = (float *) malloc(3*3*3* sizeof(float));
  ...
  // AVG GLOBAL POOLING
  t = clock();
  img_segunda_conv_ptr = Avg_Pooling_Completo_por_canal(img_conv_1x1_S1_ptr, NUM_FILAS, NUM_COLUMNAS, NUM_CANALES, STEP_FILA, STEP_COLUMNA, PADDING, img_segunda_conv_ptr);
  t = clock() - t;
  time_taken = ((double)t)/CLOCKS_PER_SEC; // in seconds
  ...
  printf("\n======== Avg_Pooling_Completo_por_canal(ReLU(suma_vector_con_tensor_por_canales(Convolucion_1x1_S1(IMAGEN INICIAL CON PADDING 1 POR CADA LADO, Wc1), bc1))) ========\n");
  ...
  printf("\n======== producto_tensores(Avg_Pooling_Completo_por_canal(ReLU(suma_vector_con_tensor_por_canales(Convolucion_1x1_S1(IMAGEN INICIAL CON PADDING 1 POR CADA LADO, Wc1), bc1))), Wn1) ========\n");
  ...
  printf("\n======== suma_vector_con_tensor_por_canales(producto_tensores(Avg_Pooling_Completo_por_canal(ReLU(suma_vector_con_tensor_por_canales(Convolucion_1x1_S1(IMAGEN INICIAL CON PADDING 1 POR CADA LADO, Wc1), bc1))), Wn1), Bn1) ========\n");
  printf("suma_vector_con_tensor_por_canales() tardó %f segundos en ejecutarse \n", time_taken);
  visualizarImagen(producto_tensores_ptr, 1, 1, 10, 1, 1);
